...
//   import de.mjel.economics.money.Money;
   ...
   public class AmountFormat /*extends TextFormat*/ {
       ...
       */
      public static function setInstance(format:AmountFormat):void {
         CURRENT = format;
      }
      ...
      [Abstract]
      public function format(amount:Amount, appendable:Appendable):Appendable {
         return null;
      }
...
import de.mjel.physics.amount.Amount;
import de.mjel.physics.amount.AmountFormat;
import de.mjel.measure.parse.Cursor;
import de.mjel.measure.parse.Appendable;
import de.mjel.measure.unit.Unit;
...
//      }
      ...
      appendable.append("(");
      TypeFormat.formatNumber(value, digits, scientific, showZeros, appendable);
      appendable.append(" ± ");
      scientific = (Math.abs(error) >= 1E6) || (Math.abs(error) < 1E-6);
      showZeros = true;
      TypeFormat.formatNumber(error, _errorDigits, scientific, showZeros, appendable);
      appendable.append(") ");
      return UnitFormat.getInstance().format(amount.unit, appendable);
   }

   override public function parse(arg0:String, arg1:Cursor=null):Amount {
      if (arg1 == null) {
         arg1 = new Cursor();
      }
      var amount:Amount;
      var start:int = arg1.index;
      var value:Number;
      var unit:Unit;
      try {
         arg1.skip("(", arg0);
         value = TypeFormat.parseLong(arg0, 10, arg1);
         if (arg0.charAt(arg1.index) == " ") { // Exact! 
            arg1.skip(" ", arg0);
            unit = UnitFormat.getInstance().parseProductUnit(arg0, arg1);
            amount = Amount.valueOf(value, unit);
         }
         arg1.index = start;
         value = TypeFormat.parseNumber(arg0, arg1);
         arg1.skip(" ", arg0);
         var error:Number = 0;
         if (arg0.charAt(arg1.index) == "±") { // Error specified. 
            arg1.skip("±", arg0);
            arg1.skip(" ", arg0);
            error = TypeFormat.parseNumber(arg0, arg1);
         }
         arg1.skip(")", arg0);
         arg1.skip(" ", arg0);
         unit = UnitFormat.getInstance().parseProductUnit(arg0, arg1);
         amount = Amount.valueOf(value, unit, error);
      }
// TODO: Fix error type
      catch (e:Error/*ParseException*/) {
         arg1.index = start;
//         arg1.errorIndex = e.getErrorOffset();
         return null;
      }
      return amount;
   }
...
//      }
      ...
      var scientific:Boolean = (Math.abs(value) >= 1E6) || (Math.abs(value) < 1E-6);
      var showZeros:Boolean = true;
      //TextBuilder tb = TextBuilder.newInstance();
      var tb:Appendable = new Appendable()
      TypeFormat.formatNumber(value, digits, scientific, showZeros, tb);
      var endMantissa:int = 0;
      for (; endMantissa < tb.toString().length; endMantissa++) {
...
//      }
      ...
      var scientific:Boolean = (Math.abs(value) >= 1E6) || (Math.abs(value) < 1E-6);
      var showZeros:Boolean = true;
      TypeFormat.formatNumber(value, digits, scientific, showZeros, arg1);
      arg1.append(" ");
      return UnitFormat.getInstance().format(arg0.unit, arg1);
   }
...
class TypeFormat {
   ...
   public static function parseBoolean(value:String, cursor:Cursor=null):Boolean {
      var start:int = (cursor != null) ? cursor.index : 0;
   ...
   public static function parseLong(value:String, radix:int=10, cursor:Cursor=null):Number {
      ...
      for (; i < end; i++) {
         ...
         var digit:int = (c.charCodeAt() <= "9".charCodeAt()) ? c.charCodeAt() - "0".charCodeAt()
            : ((c.charCodeAt() <= "Z".charCodeAt()) && (c.charCodeAt() >= "A".charCodeAt())) ? c.charCodeAt()- "A".charCodeAt() + 10
            : ((c.charCodeAt() <= "z".charCodeAt()) && (c.charCodeAt() >= "a".charCodeAt())) ? c.charCodeAt() - "a".charCodeAt() + 10 : -1;
   ...
   public static function parseNumber(value:String, cursor:Cursor=null):Number {
      const start:int = (cursor != null) ? cursor.index : 0;
      ...
      if (((c.charCodeAt() < "0".charCodeAt()) || (c.charCodeAt() > "9".charCodeAt())) && (c != ".")) {
      ...
      while (true) {
         digit = c.charCodeAt() - "0".charCodeAt();
         if ((digit >= 0) && (digit < 10)) {
   ...
   private static function match(str:String, value:String, start:int, length:int):Boolean {
   ...
   public static function format(value:Object, appendable:Appendable):Appendable {
      if (value is Boolean) {
         ...
         if ((value as Number) % 1 == 0) {
            return formatInteger(value as Number, 10, appendable);
         }
         else {
            const scientific:Boolean = (Math.abs(value as Number) >= 1E7) ||
               (Math.abs(value as Number) < 0.0001);
            return formatNumber(value as Number, -1, scientific, false, appendable);
         }
      }
      else {
    ...
    * @return the specified <code>Appendable</code> object.
    */
   public static function formatInteger(value:Number, radix:int=10, appendable:Appendable=null):Appendable {
      if (appendable == null) {
      ...
      if (radix == 10) {
         return appendable.append(value); // Faster.
      }
      if (radix < 2 || radix > 36) {
   ...
   /**
    * Appends the textual representation of the specified <code>Number</code>
    * according to the specified formatting arguments.
    *
    * @param value the <code>Number</code> value.
    ...
    * @param appendable the <code>Appendable</code> to append.
    * @return <code>TypeFormat.format(value, digits, scientific, showZero, appendable)</code>
    * @throws ArgumentError if <code>(digits &gt; 19)</code>)
    */
   public static function formatNumber(value:Number, digits:int=-1, scientific:Boolean=false, showZero:Boolean=false, appendable:Appendable=null):Appendable {
      if (appendable == null) {
         appendable = new Appendable();
      }
      if (digits > 19) {
      ...
      if (digits < 0) { // Use 16 or 17 digits.
         // Try 17 digits.
         var m17:Number = Math.pow(value, (17 - 1) - e);
      ...
      if (scientific || (e >= digits)) {
         ...
         appendable.append("0".charCodeAt() + k);
         m = m - pow10 * k;
   ...
   public static function digitLength(value:Number):int {
      ...
      return (value >= 100000000000000) ? (value >= 10000000000000000) ? (value >= 1000000000000000000) ? 19
         : (value >= 100000000000000000) ? 18 : 17
         : (value >= 1000000000000000) ? 16 : 15
         : (value >= 100000000000) ? (value >= 10000000000000) ? 14
         : (value >= 1000000000000) ? 13 : 12
         : (value >= 10000000000) ? 11 : 10;
   }
